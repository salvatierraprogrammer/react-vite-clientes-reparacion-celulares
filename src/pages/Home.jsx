...
import { Container, Button, Typography, Grid, CircularProgress } from '@mui/material'; 
      ...
      {initialLoading ? (
        ...
        <>
          {clients.filter(c => c.estado === 'pendiente').length === 0 ? (
            <Typography sx={{ mt: 3 }}>No hay clientes pendientes.</Typography>
          ) : (
            <Grid container spacing={2}>
              {clients
                .filter(client => client.estado === 'pendiente')
                .map(client => (
                  <ClientCard
                    key={client.id}
                    client={client}
